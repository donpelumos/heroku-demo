<?php
/**
  * Script to get temporary location of the user
  *
  ...
  * 
  * @return JSON success,coordinates on success or Error status on failure.
  ...
  */
  ...
  if(!$dbq->get(query_live("SELECT temp_coordinates from users Where id = ?"),[$user]) ){
    pekky_set_failure_state(-1,$dbq->err_msg);
    exit(pekky_print_json_state());  
  }
  //now do the coordinates proper
  $coord = json_coord($dbq->record[0]['temp_coordinates']);
  pekky_set_success_state();
  pekky_print_json_state($coord);  
