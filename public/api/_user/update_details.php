<?php
  ...
  *
  * It accepts the parameter fullname,email,phone,address,user_id,coord(optional),dont_send_mail(optional although it makes sense for mail to be sent if an email was changed :)) using GET
 ...
 //Check if the email is changed. 
  $email_changed = false;
 if( !$dbq->get(query_live("select id,email from users WHERE id = ? "),[$user_id])){
    //error
  ...
  if(!empty($coord)){
	if(!is_valid_coord($coord)){ // Invalid
		pekky_set_failure_state(0,"coordinate format incorrect, check the documentation for valid format.");
		exit(pekky_print_json_state());
	}
    $coord_data = array('coordinates'=>'?');
    $coord_binding = array($coord);
    $data = array_merge($data,$coord_data);
   $binding = array_merge($binding,$coord_binding);
  }
 
 //always make the user id the last bind value :)
 $binding = array_merge($binding,array($user_id));
if(!$dbq->change("users",$data,query_live("Where id = ?"), $binding) ){
    pekky_set_failure_state(-1,$dbq->err_msg);
    exit(pekky_print_json_state());
}
...
if($dbq->rows_affected == 1){//successful
	if($email_changed && !$n_send_mail){//email changed, && means you should send default mail
		...
		Please make sure you can access that email address, and it works well, because once you approve the change, incase you forget your password, a reset link will be sent to your email associated with your account.
		Please <a href=\"".$link."\">click here</a> or use the link below to confirm approval<br/>".$link."
		<br/><br/>Please if you do not remember initiating this action, do not click the link. Do well to change your password also. ";
