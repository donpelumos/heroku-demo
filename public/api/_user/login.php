<?php
 ...
 if( !$dbq->get(query_live("select id from users WHERE active = 0 and email = ? and password = ? "),[$email,$password]) ){
  //error
...
exit(pekky_print_json_state());
 ...
 if( !$dbq->get(query_live("select id,fullname,email,phone from users WHERE email = ? AND password = ? "),[$email,$password,true])){
    //error
    pekky_set_failure_state(-1,$dbq->err_msg);
  exit(pekky_print_json_state());
  }
