<?php
 ...
 //require "vendor/pp/DB/DBCon.class.php";
  $email = isset($_GET["login_id"]) ? $_GET["login_id"] : false;
 ...
 if( !$dbq->get(query_live("select id from dispatchers WHERE active = 0 and login_id = ? and password = ? "),[$email,$password]) ){
  //error
...
exit(pekky_print_json_state());
 ...
 if( !$dbq->get(query_live("select id,login_id,fullname,email,phone,date_time,date_time_updated from dispatchers WHERE login_id = ? AND password = ? "),[$email,$password,true])){
    //error
    pekky_set_failure_state(-1,$dbq->err_msg);
  exit(pekky_print_json_state());
  }
